...
sequenceDiagram
  ...
  Front->>User: POST /login (credentials)
  User-->>Front: JWT Token

  Front->>Product: POST /products (with JWT)
  Product->>User: GET /users/{id} (validate role)
  User-->>Product: 200 OK (SELLER)
  Product-->>Kafka: Event PRODUCT_CREATED
  Product-->>Front: 201 Created (productId)

  Front->>Media: POST /media/upload (file, productId, JWT)
  Media->>Product: GET /products/{id} (verify owner)
  Product-->>Media: 200 OK
  Media-->>Front: 201 Created (image URL)
